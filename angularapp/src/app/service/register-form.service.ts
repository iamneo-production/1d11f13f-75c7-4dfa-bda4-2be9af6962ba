...
export class RegisterFormService {
  ...
  saveFormData(formData: any) {
    return this.http.post<any>('https://8080-dedafedcfdcefbeacfaceadeaeaadbdbabf.project.examly.io/home/register', formData);
  }
 
  checkEmailExists(email: string): Observable<boolean> {
    return this.http
      .get<boolean>(`https://8080-dedafedcfdcefbeacfaceadeaeaadbdbabf.project.examly.io/home/check-email/${email}`)
      .pipe(
        catchError(() => {
  ...
  checkphoneNumberExists(phoneNumber: string): Observable<boolean> {
    return this.http
      .get<boolean>(`https://8080-dedafedcfdcefbeacfaceadeaeaadbdbabf.project.examly.io/home/check-phoneNumber/${phoneNumber}`)
      .pipe(
        catchError(() => {
          // Handle error if needed
          return of(false);
